...
> かつて Ollama v0.17.0 〜 v0.17.5 までのバージョンにおいて、`glm-ocr` モデルを使用した際の OCR 結果が異常（空白等）になる問題が発生していましたが、**Ollama v0.17.6 以降でこの問題は修正されています。**
> 
> 現在最新の Ollama を使用していれば問題ありませんが、もし v0.17.0 〜 v0.17.5 をお使いの場合は、**v0.17.6 以上へのアップデート**を推奨します。
...
| `extra_text_extensions` | テキストとしてそのまま読み込む追加の拡張子リスト（例: `[".conf", ".env"]`） |
...
> [!TIP]
> **複数の API キーを指定可能**  
> `x-api-key` ヘッダーや URL クエリパラメータには、カンマ区切りで複数の API キー（例: `key1,key2`）を指定できます。この場合、指定されたすべてのキーに紐づく `allowed_roots` がマージ（和集合）され、複数のグループの文書を横断的に検索できるようになります。
...
> [!NOTE]
> Ollama v0.17.0 〜 v0.17.5 において `glm-ocr` モデルに非互換の不具合が確認されていましたが、この問題は **Ollama v0.17.6 以降で修正されました。**

**対処法:**
- Ollama のバージョンを **v0.17.6 以上**にアップデートしてください。
- アップデートが困難な場合は、OCR エンジンを `"paddleocr"` に切り替えるか、Ollama を v0.16.3 にダウングレードしてください。

なお、`file_converter.py` は OCR 出力が明らかに異常な場合（空文字・極端に短い出力・同一文字の繰り返しなど）を自動検出し、OCR 失敗として空のテキストを返すよう対応しています。ログに `OCR validation failed:` のメッセージが出た場合は、Ollama のアップデートをご検討ください。
...
通常、HTTP ヘッダーに日本語を含めると文字化け（Mojibake）が発生しますが、本サーバーは **CLINE から送られる日本語ヘッダーを自動的にデコードして復元する機能**を備えています。

そのため、多くの場合で特段の配慮なく `x-roots` 等に日本語を記載できます。

> [!TIP]
> もしヘッダーでの指定で正しく動作しない場合は、**URL のクエリパラメータ**に記載することでより確実に設定可能です。
...
| `x-roots` (ヘッダー) | デフォルトで検索するルートフォルダ | ✅（Mojibake自動修復）|
| `roots` (URL クエリ) | 同上（URLパラメータなら確実） | ✅ |
| `x-categories` (ヘッダー) | デフォルトで検索するサブフォルダ | ✅（Mojibake自動修復）|
| `categories` (URL クエリ) | 同上（URLパラメータなら確実） | ✅ |
...
| テキスト | `.txt` `.md` **+ `extra_text_extensions` で指定したもの** | そのまま読み込み |
| 設定ファイル等 | `.json` `.xml` `.yaml` `.yml` `.toml` `.ini` `.env` `.log` `.conf` | そのまま読み込み |
